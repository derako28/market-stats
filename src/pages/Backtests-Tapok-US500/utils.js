...
export const getChartConfigForBacktest = (
  ...
  return {
    ...chartConfig(getData, data, propertyRR),
    width: width,
    height: height,
  };
};
...
export const getDataLine = (data) => {
  ...
  return Object.keys(prepareData).map((key) => {
    return { pureDate: key, date: new Date(key), profit: prepareData[key] };
  });
};
...
export const getDataLine3 = (dataBefore, dataAfter) => {
  ...
  return Object.keys(prepareData).map((key) => {
    return {
      date: new Date(moment(key, "MM-DD-YYYY").format()),
      profit: prepareData[key],
    };
  });
};
...
export const getDataLine2 = (data) => {
  ...
  const prepareData = data.reduce((acc, { date, result }) => {
    ...
    return [
      ...acc,
      { date: new Date(moment(date, "MM-DD-YYYY").format()), profit: profit },
    ];
  }, []);
...
export const getStatistic = (data = []) => {
  const getWinRateInfo = () => {
    return ((getWinRate(data, "1r")[0].amount / data.length) * 100).toFixed(0);
  };
...
export const assignResult = (data, property = "1r") => {
  return data.map((item) => ({
    ...item,
    result: item[property] === "Yes" ? "win" : "lose",
  }));
};
...
export const sortByDate = (data) => {
  return data.sort((a, b) => {
    const dateA = new Date(a.date); // Преобразуем дату в формат YYYY-MM-DD
    const dateB = new Date(b.date); // Преобразуем дату в формат YYYY-MM-DD
