...
export const getDataLine = (data) => {
  ...
  const prepareData = data.reduce((acc, { date, result, rr }) => {
    if (result === "win") {
      profit = profit + rr;
    } else {
      profit--;
  ...
  return Object.keys(prepareData).map((key) => {
    return {
      pureDate: key,
      date: new Date(moment(key, "DD-MM-YYYY").format()),
...
export const getDataLine2 = (data) => {
  ...
  const prepareData = data.reduce((acc, { date, result }) => {
    ...
    return [
      ...acc,
      { date: new Date(moment(date, "DD-MM-YYYY").format()), profit: profit },
    ];
  }, []);
...
export const getStatistic = (data = []) => {
  ...
  const getProfit = () => {
    return data?.reduce((acc, { result, rr }) => {
      if (result === "win") {
        acc = acc + rr;
      } else {
        acc--;
...
export const limitLoseStreak = (
  ...
  data.forEach((item) => {
    ...
    if (item.result === "lose" && loseCountPerDay[loseDate] < maxLosePerDay) {
...
export const assignRR = (data) => {
  ...
  // Проставляем rr для каждой группы
  const result = Object.values(groupedByDate).flatMap((group) => {
    if (group.length === 1) {
      // Если в день одна позиция, rr = 1
      group[0].rr = 1;
    } else {
      // Разбиваем позиции на группы по направлению
      const directions = group.reduce((acc, item) => {
      ...
      // Если оба направления присутствуют, rr = 1 для каждой позиции
      if (Object.keys(directions).length > 1) {
        group.forEach((item) => (item.rr = 1));
      } else {
        // Если только одно направление, проставляем rr = 1, 2, ...
        const directionGroup = directions[Object.keys(directions)[0]];
        directionGroup.forEach((item, index) => {
          item.rr = index + 1;
        });
      }
    }
