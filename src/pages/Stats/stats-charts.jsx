...
const columns = [
  ...
  // { id: 'IB_Breakout', title: 'IB Breakout', minWidth: 110 },
  // { id: 'Breakout_High', title: 'Breakout High', minWidth: 110 },
  // { id: 'Breakout_Low', title: 'Breakout Low', minWidth: 110 },
];
...
export const StatsCharts = () => {
  ...
  const getConfig = () => {
    ...
    Object.keys(OPENS).map((key) => {
      data.open[OPENS_LABEL[key]] = tableData.filter(
        (item) => item.open === OPENS_LABEL[key],
      ).length;
    });

    Object.keys(OPENING_TYPES).map((key) => {
      data.opening_type[OPENING_TYPES[key]] = tableData.filter(
        (item) => item.opening_type === OPENING_TYPES[key],
      ).length;
    });

    Object.keys(DAY_TYPES_LABEL).map((key) => {
      data.type_day[DAY_TYPES_LABEL[key]] = tableData.filter(
        (item) =>
          item.type_day.toLowerCase() === DAY_TYPES_LABEL[key].toLowerCase(),
      ).length;
    });
